...
df <- list.files(path=dir, pattern="*.csv", full.names = TRUE) %>% 
  lapply(RemoveWhiteSpace) %>% 
  bind_rows 

dir2 <- "../results/experiment1/jsp"

dfControl <- list.files(path=dir2, pattern="*.csv", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  bind_rows 

considered <- c("la01","la06","la11","la16","la21","la26","la31","la36")
dfControl <- dfControl %>% filter(seedNum == 1603, instance %in% considered) %>%
  mutate(plsCase = "control") %>% select(-seedNum)
...
# Include instance sizes and best values
bests <- c(666, 926, 1222, 945, 1046, 1218, 1784, 1268)

sizes <- c("10x5","15x5","20x5","10x10","15x10","20x10","30x10","15x15")
...
for (i in 1:8) {
  keys[i] <- paste0("la", sprintf("%02d", 5*(i-1)+1))
...
# Nsizes boxplot
p <- ggplot(dfNSizes, aes(x = Nsize, fill = nbhOperator)) +
...
```{r fig7, fig.cap="JSPTWT Total number of neighbors by operator and instance size", message=FALSE, warning=FALSE}
# Count neighborhood sizes
dfNSizes <- df %>% filter(neighborIdx == 1)
