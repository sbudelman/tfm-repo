...
cet <- function(arr, path) {
  
  # Get cri#tical blocks from path
  blocks <- cblocks(arr, path)
...
TS <- function(edges, arr, listSize=5, maxIter=20, maxNoImprove = maxIter, toPlot=FALSE, verbose=FALSE, Nstrategy = "cet"){

  # Initialise global best
  longPath <- longest_path(edges, arr, weighted = TRUE)
  bestQ <- edges
  bestCmax <- longPath$makespan
  bestPath <- longPath$path
  
  # Initialise iter best
  path <- bestPath
  bestN <- edges
  ...
  while(iter < maxIter & nimp < maxNoImprove){
    ...
    for(v in 1:nrow(moves)){
      ...
      longPath <- longest_path(Q, arr, weighted = TRUE)
      #toc()
    ...
    path <- longest_path(bestN, arr, weighted=TRUE)$path
...
# ---- GRASP ----
GRASP <- function(arr, alpha = 1,
                  maxIter = 10, maxNoImprove = maxIter, toPlot = FALSE,
                  Nstrategy = "cet"){
  ...
  while(i < maxIter & nimp < maxNoImprove) {
    ...
    constructor <- grasp_build(arr, alpha)
    newEdges <- constructor$edges
    
    toc()
    
    # Check whether initial solution is reasonably good compared to current best
    longPath <- longest_path(newEdges, arr, weighted = TRUE)
    
    
    if( longPath$makespan / 1.1 > bestCmax ) {
    ...
    x <- TS(newEdges, arr, toPlot = toPlot,
...
grasp_build <- function(arr, alpha = 1, fast = FALSE){
  # Define constants
  nJobs <- nrow(arr)
  nTasks <- ncol(arr)
  edges <- arr_to_edges(arr)
  sink <- nJobs*nTasks + 1
  ...
  while(nrow(scheduled) < sink){
    
    # tic("3")
    # 3: Identify all schedulable operations
    tmp <- conjunctive[!conjunctive[,"from"] %in% scheduled[,1],]
    if(!is.null(nrow(tmp))){
